...
export default function AssistantsPage() {
  ...
  async function loadAssistants() {
    ...
    if (assistantResult.error) {
      console.error(
        'LOAD ASSISTANTS ERROR:',
        assistantResult.error
      )

      setError(assistantResult.error.message)
      setLoading(false)
      return
    }

    const assistantRows = assistantResult.data || []
    ...
    const businessIds = assistantRows
      .map(function (assistant) {
        return assistant.business_id
      })
      .filter(Boolean)
    ...
    const formattedAssistants: Assistant[] =
      assistantRows.map(function (assistant) {
        return {
          id: assistant.id,
          name: assistant.name,
          ...
          business_name:
            businessMap[assistant.business_id] || 'Business',
        }
      })
          ...
          <div className="mt-10 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            {assistants.map(function (assistant) {
              ...
              return (
                <div
                  key={assistant.id}
                  className="grid grid-cols-12 items-center border-b border-slate-100 px-6 py-5"
                >
                  <div className="col-span-5">
